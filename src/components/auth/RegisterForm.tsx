...
function PasswordStrength({ password }: { password: string }) {
  const getStrength = () => {
    ...
    if (score <= 3)
      return { level: 2, label: "Fair", color: "bg-accent-amber" };
...
export default function RegisterForm() {
  ...
  const { register, loginWithGoogle, error, isLoading, clearError } =
    useAuthStore();
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    if (success) {
      ...
      setFormError("root", { message: error || "Registration failed" });
    }
  };
      ...
      >
        ...
        {(errors.root || error) && (
          <motion.div
            ...
            className="p-3 bg-accent-red/10 border border-accent-red/20 rounded-lg text-sm text-accent-red"
          >
